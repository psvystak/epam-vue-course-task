<template>
  <transition name="fade">
    <div v-if="moviePopup" class="popup">
      ...
      <div class="input-wrapper">
        ...
        <div class="release-date small-input">
          ...
          <datepicker ref="datepicker" v-model="inputsData.release_date" :enableTimePicker="false"
                      class="datepicker" dark
                      placeholder="Select Date" @update:modelValue="setDate"
          name="releaseDate">
            <template #input-icon>
              <svg v-if="!inputsData.release_date" class="calendar-icon" fill="none" height="22" viewBox="0 0 24 22"
      ...
      <div class="input-wrapper">
        ...
        <div class="rating small-input">
          ...
          <input id="rating" v-model="inputsData.rating" placeholder="Insert Rating" type="text">
        </div>
      </div>
      <div class="input-wrapper">
        <div class="large-input genre">
          <p class="label" @click="toggleDropdown">GENRE</p>
...
const AddMovie = defineComponent({
  ...
  computed: {
    ...mapGetters(["moviePopup", "editMode"]),
    title() {
      if (!this.editMode) {
        return "ADD MOVIE";
      } else {
  ...
  methods: {
    ...
    setDate(date: any) {
      this.inputsData.formattedReleaseDate = new Date(Date.parse(date)).toLocaleDateString("ua-UA").split(".").reverse().join("-");
    },
    reset() {
      if (!this.editMode) {
        //@ts-ignore
    ...
    submit() {
      //@ts-ignore
      if (Object.keys(this.inputsData).some((input) => this.inputsData[input] === "")) {
    ...
    addGenre(id: number) {
      if (this.genres[id].selected) {
        let selectedId = this.selectedGenres.findIndex((name) => name === this.genres[id].name);
        this.selectedGenres.splice(selectedId, 1);
      } else {
        //@ts-ignore
        this.selectedGenres.push(this.genres[id].name);
        //@ts-ignore
        this.selectedGenres = this.selectedGenres.sort((a, b) => a.localeCompare(b));
      }
